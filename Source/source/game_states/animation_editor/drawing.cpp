...
void animation_editor::draw_canvas() {
    ...
    if(draw_leader_silhouette) {
        float x_offset = 32;
        if(loaded_mob_type) {
            x_offset += loaded_mob_type->base_data.radius;
        }
...
void animation_editor::draw_side_view_sprite(const sprite* s) {
    ...
    if(loaded_mob_type) {
        ...
        min.y = loaded_mob_type->base_data.height;
    } else {
        min.y = max.x - min.x;
    }
    if(color.a == 0) {
...
void animation_editor::draw_top_down_view_mob_radius(mob_type* mt) {
    al_draw_circle(
        0, 0, mt->base_data.radius,
        al_map_rgb(240, 240, 240), 1.0f / game.cam.zoom
    );
    if(mt->base_data.rectangular_dim.x != 0) {
        al_draw_rectangle(
            -mt->base_data.rectangular_dim.x / 2.0, -mt->base_data.rectangular_dim.y / 2.0,
            mt->base_data.rectangular_dim.x / 2.0, mt->base_data.rectangular_dim.y / 2.0,
            al_map_rgb(240, 240, 240), 1.0f / game.cam.zoom
        );
    }
