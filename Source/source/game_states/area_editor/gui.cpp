...
void area_editor::process_gui_mob_script_vars(mob_gen* m_ptr) {
    ...
    //Health property.
    float max_health = m_ptr->type->base_data.max_health;
    ...
    set_tooltip(
        ...
        "The object type's default is " + f2s(m_ptr->type->base_data.max_health) + ".\n"
        "(Variable name: \"max_health\".)",
        "",
        WIDGET_EXPLANATION_DRAG
    );
    
    if(max_health != m_ptr->type->base_data.max_health) {
