...
void area_data::clear() {
    ...
    mission.points_per_enemy_death_point = 0;
    mission.points_per_enemy_collection_point = 0;
...
void area_data::clone(area_data &other) {
    ...
    other.mission.points_per_enemy_death_point = mission.points_per_enemy_death_point;
    other.mission.points_per_enemy_collection_point = mission.points_per_enemy_collection_point;
...
void area_data::load_mission_data_from_data_node(data_node* node) {
    ...
    rs.set("mission_points_per_enemy_death_point", mission.points_per_enemy_death_point);
    rs.set("mission_points_per_enemy_collection_point", mission.points_per_enemy_collection_point);
...
void area_data::save_mission_data_to_data_node(data_node* node) {
    ...
    if(mission.grading_mode == MISSION_GRADING_MODE_POINTS) {
        ...
        if(mission.points_per_enemy_death_point != 0) {
            node->add(
                new data_node(
                    "mission_points_per_enemy_death_point",
                    i2s(mission.points_per_enemy_death_point)
                )
            );
        }
        if(mission.points_per_enemy_collection_point != 0) {
            node->add(
                new data_node(
                    "mission_points_per_enemy_collection_point",
                    i2s(mission.points_per_enemy_collection_point)
                )
            );
        }
